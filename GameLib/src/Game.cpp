...
#include "GameFiles.h"
void Scene::Init(const std::string& fp)
{
	...
	for (int i = 0; i < rmfSampleCount; ++i)
	 ...
	 rmfForwardDirs = KGR::RMF::EstimateForwardDirs(rmfPoints);
	 rmfFrames = KGR::RMF::BuildFrames(rmfPoints, rmfForwardDirs);
...
void Scene::UpdateGame(float dt)
{
	...
	//Camera control
	{
		...
		for (auto& e : es)
		{
			...
			glm::quat yawQ = glm::angleAxis(-glm::radians(delta.x * dt * 100.0f), glm::vec3(0, 1, 0));
			glm::quat pitchQ = glm::angleAxis(-glm::radians(delta.y * dt * 100.0f), transform.GetLocalAxe<RotData::Dir::Right>());
			transform.SetOrientation( glm::normalize(yawQ * pitchQ * transform.GetOrientation()));
		}
	}
	...
	//Update bullet
	{
		...
		for (auto& e : view)
		{
			...
			newPos += bullet.direction * bullet.speed * dt;
	...
	//Update enemy
	{
		...
		for (auto& enemyEntity : enemies)
		{
			...
			glm::vec3 newPos = enemyAI.Update(dt, enemyTransform.GetPosition(), enemy.speed);
...
void Scene::Run(const KGR::Tools::Chrono<float>::Time& fixedTime)
{
